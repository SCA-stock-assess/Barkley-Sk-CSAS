...
ref_pts_alt <- tribble(
  ...
  "G[RMD]", "Great Central", 0.51, 0.35, 0.64, 115286, 92950, 146123,
  "G[RMD]", "Sproat", 0.61, 0.45, 0.73, 85395, 70993, 106639,
  "50^e~percentile", "Hucuktlis", 0.59, 0.14, 0.81, 13948, NA_real_, NA_real_
) |> 
...
make_kobe_p <- function(xy_data, reflines_data, endyrs_data, facet_rows) {
  ...
  ggplot(
    ...
    geom_labelhline(
      data = reflines_data,
      aes(
        label = paste0("U[RMD]==", 100*round(Umsy_q50, 2), "*\'%\'"),
        yintercept = 1
      ),
      hjust = 0.95,
      colour = "grey50",
    ...
    labs(
      colour = "Année de montaison",
      y = expression(frac("Taux d’exploitation", U[RMD])), 
      x = expression(frac(Abondance~des~géniteurs,G[RMD])),
      parse = TRUE
    ) +
  ...
  pwalk(
    function(kobe_p, group, width) ggsave(
      ...
      filename = here(
        ...
        "Plots",
        "Français",
        paste0("fr_Kobe_plots_", group, ".png")
      ),
      width = width,
      height = 5,
      units = "in",
