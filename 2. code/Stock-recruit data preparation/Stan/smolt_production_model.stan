data {
  ...
  // Companion vector for count data: 1 if A_total and A1_obs are observed in that year, 
  // 0 otherwise (missing values coded as -999)
...
transformed parameters { // deterministic statements
  ...
  array[Y] real<lower=0> M; // Second winter mortality for holdovers
  ...
  array[Y] real<lower=0, upper=1> p1; // Age-1 proportion
  array[Y] real<lower=0, upper=1> p2; // Age-2 proportion
...
model {
  ...
  N2_init ~ lognormal(N2_init_prior, N2_init_sigma_prior);
