...
parameters {
  ...
  vector[2] mu_thetaM;                // population means on logit scale if desired
  vector<lower=0>[2] sigma_thetaM;    // sd for theta and M
  cholesky_factor_corr[2] L_Omega;    // 2x2 cholesky of correlation
  matrix[2, Y] z_thetaM;              // std normals
  ...
  // True average weights for O1 and O2 fish in each year (log space)
  // values constrained to prevent sampling at unrealistic weights
  array[Y] real<lower=-3, upper=3.14> w1_ln; 
  array[Y] real<lower=-3, upper=3.14> w2_ln;
...
transformed parameters {                 
  ...
  array[Y] real<lower=0> w1;             // Age-1 smolt average weight (g)
  array[Y] real<lower=0> w2;             // Age-2 smolt average weight (g)
  ...
  for (y in 1:Y) {
    thetaM_raw[, y] = (diag_pre_multiply(sigma_thetaM, L_Omega) * z_thetaM[, y]);
  }
  // assign into theta and M
  for (y in 1:Y) {
    real theta_raw_y = thetaM_raw[1, y]; // use in inv_logit(mu_theta + theta_raw_y)
    real M_raw_y     = thetaM_raw[2, y]; // use in inv_logit(mu_M + M_raw_y)
    theta[y] = inv_logit(mu_thetaM[1] + thetaM_raw[1, y]);
    M[y]     = inv_logit(mu_thetaM[2] + thetaM_raw[2, y]);
  }
  ...
  for (y in 1:Y) {
    ...
    p1[y] = O1[y] / N_lake[y];
    p2[y] = O2[y] / N_lake[y];
    // annual average smolt weight on natural scale (g)
    w1[y] = exp(w1_ln[y]);
    w2[y] = exp(w2_ln[y]);
...
model {
  ...
  phi_raw ~ normal(logit((0.3+1)/2), 0.6);        // favors positive persistence
  ...
  // Correlated hierarchical deviations - z scores
  sigma_thetaM ~ normal(0, 0.3) T[0,];
  L_Omega ~ lkj_corr_cholesky(2.0);
  to_vector(z_thetaM) ~ std_normal();
  ...
  for (y in 1:Y) {
    ...
    // For cases where WO1_n or WO2_n = 1, use reasonably wide fixed variance
    if (is_observed_WO1[y] == 1 && WO1_n[y] == 1 ) {
    WO1_ln_mean[y] ~ normal(w1_ln[y], 0.1);
    }
    if (is_observed_WO2[y] == 1 && WO2_n[y] == 1 ) {
    WO2_ln_mean[y] ~ normal(w2_ln[y], 0.1);
    }
  }
}
...
generated quantities{
  // correlation between theta and M
  corr_matrix[2] Omega = multiply_lower_tri_self_transpose(L_Omega);
  real rho_theta_M = Omega[1, 2];   

  real mu_theta = mu_thetaM[1]; 
  real mu_M = mu_thetaM[2]; 
  real sigma_theta = sigma_thetaM[1]; 
  real sigma_M = sigma_thetaM[2];
}
